...
This allow you to mocking libraries, like Mockito.
All methods in the used `android.jar` file throw exceptions (by default). This is to make sure your unit tests only test your code and do not depend on any particular behavior of the Android platform. If you want specific behavior you can use a mocking framework to replace these call.
...
image::img/runUnitTestInAndroidStudio10.png[Running Unit tests in Android Studio]

[[androidtesting_projectstructure_testresults]]
=== Location of test reports

The rest reports are created in the `app/build/reports/tests/debug/`directory. 
